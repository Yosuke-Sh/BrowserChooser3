...
### 起動位置設定（v0.2.2以降）
マルチモニター環境で、選択画面をどこに表示するかを設定できます。

1. 「Display」タブの「Startup Position」ドロップダウンから選択：
   - **マウスカーソルのある画面の中央**（既定）: URLをクリックした瞬間にカーソルがある画面に表示されます
   - **プライマリ画面の中央**: 常にプライマリモニターの中央に表示されます（v0.2.1以前の動作）
2. タスクバーには重ならないように自動調整されます
