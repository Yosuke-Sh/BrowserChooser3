...
namespace BrowserChooser3.Tests
{
    ...
    public class MainFormTests : IDisposable
    {
        ...
        [Fact]
        public void MainForm_CancelButton_ShouldBeSetToCloseTheForm()
        {
            // Arrange & Act
            var mainForm = new MainForm();

            // Assert
            // MainFormはEscキーでの終了を想定してCancelButtonを設定している。
            // WinForms既定値のnullのままだと、Escでの終了機能が無いことを意味する。
            mainForm.CancelButton.Should().NotBeNull();
        }

        [Fact]
        public void MainForm_KeyPreview_ShouldBeEnabledForHotkeyHandling()
        {
            // Arrange & Act
            var mainForm = new MainForm();

            // Assert
            // KeyPreviewがfalseだと、ブラウザホットキーやOptionsショートカットが
            // 子コントロールにフォーカスがある場合に効かなくなる
            mainForm.KeyPreview.Should().BeTrue();
        }
